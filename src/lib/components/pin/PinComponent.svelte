    ...
    <div class="card-body bg-base-300 h-[200px] max-h-[200px] text-sm">
        <div class="grid grid-cols-12">
            ...
            <div class="col-span-10 ml-2">
                ...
                <p class="whitespace-pre-wrap break-words line-clamp-3 text-xs min-h-16">{pin.description ?? ''}</p>
                {#if mode !== 'preview'}

                    <div class="grid grid-cols-12">
                        <div class="col-span-11">
                            <div class="badge badge-success text-xs">{pin.boards?.name}</div>
                            <div class="badge text-xs ml-2">
                                {pin.profiles?.username}, {moment(pin.created_at).format('MMMM Do YYYY, h:mm:ss a')}
                            </div>
                        </div>
