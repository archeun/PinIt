...
<div class="card w-full shadow mb-4 rounded-none">
    <div class="card-body bg-base-300 h-[200px] max-h-[200px] text-sm">
        <div class="grid grid-cols-12">
            <div class="col-span-2 h-[120px] max-h-[120px] flex justify-center">
                <img class="max-h-[120px]" src={pin.image ? pin.image: 'https://placehold.co/300x200?text=No+Image+Available'}
                     alt={pin.title}>
            </div>
            <div class="col-span-10 ml-2">
                ...
                <p class="whitespace-pre-wrap break-words line-clamp-3 text-xs">{pin.description ?? ''}</p>
            </div>
        </div>
        <div class="card-actions justify-end flex">
            {#if mode !== 'preview'}
                <div class="badge badge-success text-xs pt-1">{pin.boards?.name}</div>
                <div class="badge badge-warning text-xs pt-1">{pin.profiles?.username}, {moment(pin.created_at).format('MMMM Do YYYY, h:mm:ss a')}</div>
